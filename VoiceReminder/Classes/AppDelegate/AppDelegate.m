...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if (![[NSUserDefaults standardUserDefaults] boolForKey:@"VRDeleteDatabaseCount1"]) {
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:@"VRDeleteDatabaseCount1"];
        [[NSUserDefaults standardUserDefaults] synchronize];
        [self deleteOldDBAndCreateNewOne];
    }
    else {
        [self setupCoreData];
    }
...
- (void)deleteOldDBAndCreateNewOne {
    NSLog(@"WARNING: DELETE DATABASE");
    ...
    NSArray * dirContents =[fm contentsOfDirectoryAtURL:[NSURL fileURLWithPath:applicationStorageDirectory isDirectory:YES] includingPropertiesForKeys:@[] options:NSDirectoryEnumerationSkipsHiddenFiles error:nil];
    for (NSURL * pathURL in dirContents) {
        [fm removeItemAtURL:pathURL error:&error];
    }
    if (!error) {
        [self setupCoreData];
    }
